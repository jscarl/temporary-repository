      ...
      <div class="max-w-sm lg:max-w-xl sm:w-1/2 lg:w-1/3 p-3">
        <div class="bg-white max-w-full shadow-xl rounded-md overflow-hidden relative">
          ...
          <div class="flex items-center justify-between">
            <a class="float-left p-5 text-sm text-gray-700 hover:text-pri-200" href="{{AppAlbum::getSlug('posts', $post->slug)}}">Read More</a>
            <span style="font-size: 12px;" class="p-5 float-right text-gray-700 ">{{date('d M, Y', strtotime($post->created_at))}}</span>
          </div>
        </div>
      </div>
      @endforeach
