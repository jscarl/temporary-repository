...
@section('title')
Gallery {{$album->nama_album}}
@endsection

@section('content')
<!-- Breadcumb Section -->
<section style="background-image: url('{{Voyager::image($album->thumbnail_file)}}'); transform: translateZ(-1px); overflow:hidden" class="breadcumb relative bg-blue-500 bg-center lg:bg-fixed bg-cover bg-no-repeat overflow-x-hidden flex justify-center items-center">
...
<nav class="py-3 flex justify-center text-sm md:text-lg items-center mx-auto -mb-3 md:mb-0 md:py-0 md:mt-6">
  <ul class="flex items-center flex-wrap">
    ...
    <li>
      <p class="text-gray-500 capitalize">{{$album->nama_album}}</p>
    </li>
  </ul>
</nav>
...
<section class="px-4 sm:px-8 lg:px-12 xl:px-40 mb-10 md:py-10">
  <div class="container mx-auto max-w-screen-xl">
    <div class="flex flex-wrap justify-center -mx-2" id="lightgallery">
      @foreach ($photos as $photo)

      <div class="max-w-sm lg:max-w-xl sm:w-1/2 lg:w-1/3" data-src="{{Voyager::image($photo->file)}}" data-sub-html="<h4>{{$photo->title}}</h4>
          ...
          <a class="" href="">
            <span class="absolute inset-0 hover:opacity-0 opacity-100 bg-gradient-to-t from-pri-100 cursor-pointer">
              <div class="absolute px-6 mt-40 lg:mt-32 pt-6 lg:py-6">
                <h5 class="text-lg lg:text-md text-white capitalize font-semibold">{{$photo->title}}</h5>
              </div>
            </span>
            <img class="object-cover lg:h-48 h-56 w-full bg-center" src="{{Voyager::image($photo->file)}}" alt="{{$photo->title}}">
          </a>
        </div>
      </div>
      @endforeach
